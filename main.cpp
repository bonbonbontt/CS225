...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
	    ...
	    sum = (int)original(xi,yi)->red+amount;
	    if (sum>=255) original(xi,yi)->red = 255;
	    else original(xi,yi)-> red =sum;

	    sum = (int)original(xi,yi)->green+amount;
	    if (sum>=255) original(xi,yi)->green = 255;
	    else original(xi,yi)-> green =sum;

	    sum = (int)original(xi,yi)->blue+amount;
	    if (sum>=255) original(xi,yi)->blue = 255;
	    else original(xi,yi)-> blue =sum;
